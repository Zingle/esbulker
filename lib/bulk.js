...
function start(queue) {
    ...
    async function run() {
        if (queue.length + chunks.length === 0) {
            console.debug(`queue is empty for ${queue.url}`);
            setTimeout(run, IDLE_DELAY);
            return;
        }
        ...
        try {
            ...
            console.debug(`loaded ${chunks.length} documents into ${queue.url}`);

            chunks.splice(0, chunks.length)
            size = 0;

            if (delta >= SLOW_INSERT_TIME) {
                console.debug(`slow insert into ${queue.url}`);
                backoff();
            } else {
                rampup();
            }
