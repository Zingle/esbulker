        ...
        try {
            ...
            switch (header.action) {
                case "insert":
                    ...
                    break;
                case "status":
                    const {endpoint, cutoff} = header;
                    //const q = queue(esurl, endpoint.slice(esurl.length+1));

                    if (!endpoint || !cutoff) {
                        res.statusCode = 404;
                        sendStatusText(res);
                    } else if (cutoff < q.oldest) {
...
function readStatusHeader(req, res) {
    ...
    const [endpoint, when] = decoded.split("|");

    return {action, endpoint, when: Number(when)};
}
